...
class BaseArgs:
    ...
    """perform gradient updates every N environment steps (default: every step)"""
    ...
    # LoRA parameters
    actor_lora_enable: bool = False
    """whether to enable LoRA for the actor"""
    critic_lora_enable: bool = False
    """whether to enable LoRA for the critic"""
    lora_rank: int = 96
    """LoRA rank (low-rank dimension)"""
    lora_alpha: float = 96.0
    """LoRA alpha scaling factor"""
    lora_a_init: str = "normal"
    """LoRA A matrix initializer: zero, normal, kaiming_uniform, orthogonal"""
    lora_b_init: str = "normal"
    """LoRA B matrix initializer: zero, normal, kaiming_uniform, orthogonal"""
    lora_learning_rate: float = 3e-4
    """learning rate for LoRA parameters; defaults to the base actor/critic learning rate"""
    lora_learning_rate_end: float = 3e-4
    """end learning rate for LoRA parameters; defaults to the base actor/critic end learning rate"""
    lora_weight_decay: float = 6e-4
    """weight decay for LoRA parameters"""
    kernel_init_scale: float = 0.5
    """initial scale for base weights when LoRA is enabled (column norms → sqrt(scale))"""
...
class IsaacVelocityRoughH1Args(IsaacLabArgs):
    ...
    num_steps: int = 8
    num_updates: int = 4
    buffer_size: int = 1024 * 3  # To reduce memory usage
    total_timesteps: int = 50000
