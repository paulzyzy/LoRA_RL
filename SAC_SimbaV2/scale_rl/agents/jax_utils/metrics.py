...
from flax.traverse_util import (
    flatten_dict as flax_flatten_dict,
    unflatten_dict as flax_unflatten_dict,
)
...
def _module_group(module: str) -> Optional[str]:
    if "encoder" in module:
        return "encoder"
    if "predictor" in module:
        return "predictor"
    return None


def _apply_mask_to_tree(
    tree: Params, mask: Optional[Params]
) -> Params:
    if mask is None:
        return tree
    aligned_mask = _align_mask_to_tree(tree, mask)
    return jax.tree_util.tree_map(lambda x, m: x * m, tree, aligned_mask)


def _align_mask_to_tree(
    tree: Params, mask: Params
) -> Params:
    tree_is_frozen = isinstance(tree, FrozenDict)
    mask_is_frozen = isinstance(mask, FrozenDict)
    tree_flat = flax_flatten_dict(
        flax.core.unfreeze(tree) if tree_is_frozen else tree
    )
    mask_flat = flax_flatten_dict(
        flax.core.unfreeze(mask) if mask_is_frozen else mask
    )
    aligned_flat = {}
    for key, value in tree_flat.items():
        mask_value = mask_flat.get(key)
        if mask_value is None:
            aligned_flat[key] = jnp.ones_like(value)
        else:
            aligned_flat[key] = mask_value
    aligned_tree = flax_unflatten_dict(aligned_flat)
    if tree_is_frozen:
        return flax.core.freeze(aligned_tree)
    return aligned_tree
...
def get_pnorm(
    ...
    pcount_dict: Params,
    prefix: str,
    mask_dict: Optional[Params] = None,
    ...
    """
    masked_params = _apply_mask_to_tree(param_dict, mask_dict)
    _pnorm_dict = jax.tree_util.tree_map(
        lambda x: jnp.linalg.norm(x), masked_params
    )
    ...
    for _pnorm_layer, _pnorm in pnorm_dict.items():
        ...
        module_group = _module_group(_module)
        pcount_key = _module + "/pcount_" + _layer
        if pcount_key not in pcount_dict:
            continue
        pcount = pcount_dict[pcount_key]
        eff_pnorm_total_numer += pcount * jnp.square(_pnorm)
        eff_pnorm_total_denom += pcount

        eff_pnorm_numer += pcount * jnp.square(_pnorm)
        eff_pnorm_denom += pcount
        if module_group == "encoder":
            pnorm_encoder_total += jnp.square(_pnorm)
            eff_pnorm_encoder_numer += pcount * jnp.square(_pnorm)
            eff_pnorm_encoder_denom += pcount
        elif module_group == "predictor":
            pnorm_predictor_total += jnp.square(_pnorm)
            eff_pnorm_predictor_numer += pcount * jnp.square(_pnorm)
            eff_pnorm_predictor_denom += pcount
...
def get_gnorm(
    ...
    Return:
        ...
        (CAUTION : norm values for vmapped functions (multi-head Q-networks) are summed to a single value)
    """
    masked_grads = _apply_mask_to_tree(grad_dict, mask_dict)
    _gnorm_dict = jax.tree_util.tree_map(
        lambda x: jnp.linalg.norm(x), masked_grads
    )
    # Add 'kernel', 'bias', and 'kernel+bias' norm for each layer
    ...
    for _gnorm_layer, _gnorm in gnorm_dict.items():
        ...
        eff_gnorm_total_numer += pcount * jnp.square(_gnorm)
        eff_gnorm_total_denom += pcount

        eff_gnorm_numer += pcount * jnp.square(_gnorm)
        eff_gnorm_denom += pcount
        if module_group == "encoder":
            gnorm_encoder_total += jnp.square(_gnorm)
            eff_gnorm_encoder_numer += pcount * jnp.square(_gnorm)
            eff_gnorm_encoder_denom += pcount
        elif module_group == "predictor":
            gnorm_predictor_total += jnp.square(_gnorm)
            eff_gnorm_predictor_numer += pcount * jnp.square(_gnorm)
            eff_gnorm_predictor_denom += pcount
...
def get_effective_lr(
    ...
    for _gnorm_layer, _gnorm in gnorm_dict.items():
        ...
        if ("kernel+bias" in _layer) or ("total" in _layer) or ("effective" in _layer):
            continue
        pnorm_key = _module + "/pnorm_" + _layer
        pcount_key = _module + "/pcount_" + _layer
        if pnorm_key not in pnorm_dict or pcount_key not in pcount_dict:
            continue
        eff_lr = _gnorm / pnorm_dict[pnorm_key]
        eff_lr_dict[_module + "/effective_lr_" + _layer] = eff_lr

        pcount = pcount_dict[pcount_key]
        module_group = _module_group(_module)

        eff_lr_total_numer += pcount * eff_lr
        eff_lr_total_denom += pcount
        if module_group == "encoder":
            eff_lr_encoder_numer += pcount * eff_lr
            eff_lr_encoder_denom += pcount
        elif module_group == "predictor":
            eff_lr_predictor_numer += pcount * eff_lr
            eff_lr_predictor_denom += pcount

    if eff_lr_encoder_denom > 0:
        eff_lr_dict[prefix + "_encoder/effective_lr_total"] = (
            eff_lr_encoder_numer / eff_lr_encoder_denom
        )
    if eff_lr_predictor_denom > 0:
        eff_lr_dict[prefix + "_predictor/effective_lr_total"] = (
            eff_lr_predictor_numer / eff_lr_predictor_denom
        )
    if eff_lr_total_denom > 0:
        eff_lr_dict[prefix + "/effective_lr_total"] = (
            eff_lr_total_numer / eff_lr_total_denom
        )

    def _set_total_from_norms(scope: str) -> None:
        g_key = f"{scope}/gnorm_total"
        p_key = f"{scope}/pnorm_total"
        if g_key not in gnorm_dict or p_key not in pnorm_dict:
            return
        denom = float(np.asarray(pnorm_dict[p_key]))
        if denom <= 0.0:
            return
        eff_lr_dict[f"{scope}/effective_lr_total"] = gnorm_dict[g_key] / denom

    _set_total_from_norms(prefix + "_encoder")
    _set_total_from_norms(prefix + "_predictor")
    _set_total_from_norms(prefix)
...
def get_num_parameters_dict(
    ...
    if mask_dict is not None:
        aligned_mask = _align_mask_to_tree(param_dict, mask_dict)
        _pcount_dict = jax.tree_util.tree_map(
            lambda m: np.asarray(m).sum(), aligned_mask
        )
    else:
        _pcount_dict = jax.tree_util.tree_map(
            lambda x: np.prod(x.shape), param_dict
        )
    # Add 'kernel', 'bias', and 'kernel+bias' norm for each layer
